...
module.exports = function(connect){
  ...
  function FSStore(options) {
    ...
    fs.stat(this.dir, function(err, stats) {
      ...
      if (err && err.errno != 2 && err.errno != 32 && err.errno != 34) throw err;
      if (stats && stats.isDirectory()) {
        self.client.emit('connect');
      } else {
        fs.mkdir(self.dir, 755, function(err) {
          if (err) throw err;
          self.client.emit('connect');
        });
      }
    ...
    fs.readFile(path.join(this.dir, sid + '.json'), 'UTF-8', function(err, data) {
        ...
        try {
          if (!data) {
            ...
            // Parse Error, assume no data file
           // Suppress error
           console.log('Warning, connect-fs has encountered a weird error. Skipping.');
           fn();
          }
         ...
         // Parse Error, assume no data file
         // Suppress error
         console.log('Warning, connect-fs is unable to read the session file. Skipping.');
         fn();
        }
      }
    );
